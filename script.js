...
function showPlan() {
    ...
    var vlrTotal = document.getElementById("vfl").value;
    vlrTotal = parseInt(vlrTotal)
    ...
    selectedlinhas = filterByValue(selectedlinhas, vlrTotal)
...
function filterByValue(selectedlinhas, vlrTotal){
    //selectedlinhas = Object.values(selectedlinhas).filter(x => x.vlr_total >= Math.floor(vlrTotal));
    //return selectedlinhas

    const listaOrdenada = selectedlinhas.map(faixa =>({
        ...faixa,
        diferenca:Math.abs(faixa.vlr_total - vlrTotal)
    }))
    .sort((a, b) => a.diferenca - b.diferenca)

    return listaOrdenada.slice(0, 5);
}
